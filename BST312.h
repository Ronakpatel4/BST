...
void BST_312 <ItemType>::preOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    if((t->left != NULL) && (t->right != NULL)){
        result.push_back(t->data);                                  // pushes back data from root
        preOrderTraversal(t->left, result);                         // then travels through left
        preOrderTraversal(t->right, result);                        // then through right
    }
...
void BST_312 <ItemType>::inOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    //YOUR CODE GOES HERE
    if((t->left != NULL) && (t->right != NULL)){
        inOrderTraversal(t->left, result);                         // pushes back data from left
        result.push_back(t->data);                                 // then gets root
        inOrderTraversal(t->right, result);                        // and finally right branch
    }
...
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    //YOUR CODE GOES HERE
    if((t->left != NULL) && (t->right != NULL)){
        postOrderTraversal(t->left, result);                         // pushes back data from left
        postOrderTraversal(t->right, result);                        // to right
        result.push_back(t->data);                                   // then root is final
    }
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
  ...
  move = root->left;
  while((move.left != NULL) && (move.right != NULL)){           //check both sides
      if(move.data == item){
          return true;
      }
      else if(move.data > item){
          prev = move;
          move = move.left;
      }
      else{                                                     //(move.data < item)
    ...
    move = root->right;
    while((move.left != NULL) && (move.right != NULL)){
        if(move.data == item){
            return true;
        }
        else if(move.data > item){
